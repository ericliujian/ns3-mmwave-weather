...
void MmWaveProp(double visibility, double stepTime){
...
Simulator::Schedule(Seconds(stepTime), &MmWaveProp, visibility, stepTime);
...
void computeRxPower(NetDeviceContainer devs, double stepTime) {
  ...
  outdata.open("RxPowervisibility.csv", std::ofstream::app); // opens the file
...
// }
  ...
  MmWaveProp(visibility, stepTime);
