...
int main (int argc, char *argv[])
{
  ...
  Ptr<MmWaveVehicularPropagationLossModel> propagationLossModel = CreateObject<MmWaveVehicularPropagationLossModel> ();
  Simulator::Schedule(Seconds(1*t), &MmWaveProp, 2*t);
  
  if ((2*t) % 10 == 0) {
  //double RxPower= propagationLossModel-> DoCalcRxPower( 15, n.Get (0)->GetObject<MobilityModel> (), n.Get (1)->GetObject<MobilityModel> ());
