...
ComputeSnr (Ptr<MobilityModel> txMob, Ptr<MobilityModel> rxMob, Ptr<const SpectrumValue> txPsd, double noiseFigure)
{
  ...
  // apply the pathloss
  double propagationGainDb = m_propagationLossModel->CalcRxPower (15, txMob, rxMob);
...
main (int argc, char *argv[])
{
  ...
  double txPow_dbm = 15.0; // tx power in dBm
  ...
  Config::SetDefault("ns3::ThreeGppPropagationLossModel::ParticleRadius", DoubleValue(particleradius));
  Config::SetDefault("ns3::ThreeGppPropagationLossModel::VVisibility", DoubleValue(visibility));
  Config::SetDefault("ns3::ThreeGppPropagationLossModel::HHumidity", DoubleValue(humidity));
  ...
  f << "Time[s] TxPosX[m] TxPosY[m] RxPosX[m] RxPosY[m] ChannelState SNR[dB] RxPower[dB]" << std::endl;
  f.close ();
